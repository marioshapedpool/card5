...
export default function Login() {
    ...
    if (error) {
      ...
      setErrorMsg(
        error.message.includes('invalid')
          ? t('login.errorInvalidCredentials', 'auth')
          : t('login.errorGeneric', 'auth')
      );
      return;
    }
  ...
  const handleGoogle = async () => {
    ...
    if (error) {
      console.error(error);
      setErrorMsg(t('login.errorGeneric', 'auth'));
    }
  };
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('login.passwordPlaceholder', 'auth')}
              />
            </div>
          </div>
          ...
          >
            {loading ? (
              ...
              <LogIn className="mr-2 h-5 w-5 group-hover:text-white" />
            )}
            {loading
              ? t('login.loggingInButton', 'auth')
              : t('login.loginButton', 'auth')}
          </button>
        </form>
